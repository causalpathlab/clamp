#' @title Functions related to weighted single effect regression model
#'
...
#'
#' @param X n by p matrix, centralized by
#'        \code{sapply(as.matrix(1:ncol(X)),
#'              function(j) { X[,j] - weighted.mean(x=X[,j], w=W[,j])}) }
#' @param y n by p matrix, centralized by
#'        \code{sapply(as.matrix(1:ncol(X)),
...
#'
#'  X : n by p matrix, centralized by
#'        \code{sapply(as.matrix(1:ncol(X)),
#'              function(j) { X[,j] - weighted.mean(x=X[,j], w=W[,j])}) }
#'  y: n by p matrix, centralized by
#'        \code{sapply(as.matrix(1:ncol(X)),
#'        function(j) { y - weighted.mean(x=y, w=W[,j])}) }
...
#'
get_EWR2_l <- function(X, y, W, # s=NULL,
  ...
  EWR2 <- sum(sweep(W*(y^2), 2, alpha, "*")) -
    2 * sum(sweep(W*X*y, 2, postb, "*")) + sum(sweep(W*(X^2), 2, postb2, "*"))
